...
from app.services.llm.factory import CompletionOptions, LLMFactory
from app.services.llm.factory import Message as LLMMessage
from app.services.llm.factory import Role
...
def initiate_document_ingestion(self, document_id: str) -> None:
    ...
    async def _ingest(db: Session):
        try:
            ...
            document: Optional[DocumentResponse] = await DOCUMENT_REPO.get_by_id(
                document_id, db
            )
            if not document:
            ...
            logger.info(
                f"Processing document: {document.title} (type: {document.content_type})"
            )
            ...
            summary = await _generate_document_summary(document.content, document.title)
            ...
            logger.info(
                f"Adding summary to the summary index for document {document_id}"
            )
            ...
            await DOCUMENT_REPO.set_processed(document_id, summary, chunk_count, db)
        except MaxRetriesExceededError:
            logger.error(f"Max retries exceeded for document {document_id}")
            await DOCUMENT_REPO.set_failed(
                document_id, "Processing failed after maximum retries", db
            )
        except Exception as e:
            logger.error(
                f"Failed to process document {document_id}: {e}", exc_info=True
            )
            await DOCUMENT_REPO.set_failed(document_id, str(e), db)
    ...
    try:
        ...
        logger.error(
            f"Failed to run async process for document {document_id}: {e}",
            exc_info=True,
        )
        raise
...
async def _generate_document_summary(content: bytes, title: str) -> str:
    ...
    try:
        ...
        prompt = get_prompt(
            "worker", "document_summary", title=title, content=truncated_content
        )
        ...
        response = await LLMFactory.complete(messages=messages, options=options)
...
def initiate_document_vector_deletion(self, document_id: str) -> None:
    ...
    async def _delete_vectors(db: Session):
        ...
        try:
            ...
            if not document:
                logger.warning(
                    f"Document {document_id} not found, cannot delete vectors"
                )
                return True

            knowledge_base_id = document.knowledge_base_id
            logger.info(
                f"Found document {document_id} in knowledge base {knowledge_base_id}"
            )
            ...
            logger.info(
                f"Deleting document summary from summary index for document {document_id}"
            )
            ...
            if success:
                logger.info(
                    f"Successfully deleted vectors for document {document_id} from knowledge base {knowledge_base_id}"
                )
            else:
                logger.warning(
                    f"Failed to delete vectors for document {document_id}, but continuing"
                )
            ...
            error_msg = str(e)
            logger.error(
                f"Failed to delete vectors for document {document_id}: {error_msg}",
                exc_info=True,
            )
            ...
            if "400" in error_msg and "Bad Request" in error_msg:
                logger.warning(
                    f"Pinecone API error (400 Bad Request) for document {document_id}"
                )
                ...
                if (
                    ...
                    logger.warning(
                        "This error is expected with Pinecone Serverless/Starter tiers"
                    )
                    logger.warning(
                        "The vector_store.py and pinecone_retriever.py have been updated to handle this case"
                    )
                    ...
                    logger.warning(
                        "This may be due to an invalid filter format or the document not existing in the index"
                    )
                    # We'll consider this a "success" since we can't do anything about it
                    logger.info(
                        f"Marking document {document_id} vector deletion as complete despite error"
                    )
                    return True
    ...
    try:
        ...
        logger.error(
            f"Failed to run async process for document vector deletion {document_id}: {e}",
            exc_info=True,
        )
        ...
        if retry_count >= max_retries:
            logger.warning(
                f"Max retries ({max_retries}) exceeded for document {document_id} vector deletion"
            )
            # We'll consider this a "success" to prevent the task from being stuck in the queue
            logger.info(
                f"Marking document {document_id} vector deletion as complete despite errors"
            )
            return True
...
def initiate_rag_retrieval(
    self, user_message_id: str, assistant_message_id: str
) -> None:
        ...
        try:
            ...
            for i, source in enumerate(sources):
                ...
                if "score" not in source:
                    source["score"] = (
                        1.0 if service == "tag" else source.get("similarity", 0.8)
                    )

                if "content" not in source:
                    if service == "tag":
                        source["content"] = (
                            f"Table data from {source.get('title', 'database')}"
                        )
                    else:
                ...
                if service == "questions":
                    ...
                    if "question_id" not in source:
                        ...
                        source["question_id"] = metadata.get(
            ...
            if service == "tag" and response.get("sql"):
                ...
                if response.get("results"):
                    metadata["sql_results"] = response.get("results")[
                        :5
                    ]  # Limit to first 5 results to avoid huge payload
            ...
            logger.error(
                f"Failed to process response for message {assistant_message_id}: {e}",
                exc_info=True,
            )
...
def initiate_question_ingestion(self, question_id: str) -> None:
    ...
    async def _ingest(db: Session):
        try:
            ...
            question: Optional[QuestionResponse] = await QUESTION_REPO.get_by_id(
                question_id, db
            )
            if not question:
            ...
            logger.info(
                f"Processing question: {question.question} (type: {question.answer_type})"
            )
            ...
            try:
                ...
                logger.info(
                    f"Question {question_id} successfully ingested into questions index"
                )
                ...
                logger.error(
                    f"Failed to ingest question {question_id}: {e}", exc_info=True
                )
...
def initiate_question_vector_deletion(
    self, question_id: str, knowledge_base_id: str
) -> None:
    """
    ...
    """
    logger.info(
        f"Starting question vector deletion task for question_id: {question_id}"
    )
    ...
    async def _delete_vectors(db: Session):
        try:
            ...
            vector_store = get_vector_store(
                store_type="pinecone", index_name=settings.PINECONE_QUESTIONS_INDEX_NAME
            )
            ...
            logger.info(
                f"Successfully deleted question {question_id} vectors from questions index"
            )
