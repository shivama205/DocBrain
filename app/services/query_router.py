...
class QueryRouter:
    ...
    async def route_and_dispatch(
        ...
        try:
            ...
            kb_id = (
                metadata_filter.get("knowledge_base_id") if metadata_filter else None
            )
            ...
            if force_service:
                ...
                if service not in ["rag", "tag"]:
                    logger.warning(
                        f"Invalid force_service value: {force_service}. Defaulting to RAG."
                    )
                    service = "rag"
            ...
            if service == "tag":
                ...
                response = await self.tag_service.retrieve(
                    knowledge_base_id=(
                        metadata_filter.get("knowledge_base_id")
                        if metadata_filter
                        else None
                    ),
                    query=query,
    ...
    async def _check_questions_index(
        self, query: str, knowledge_base_id: Optional[str] = None
        ...
        """
        try:
            logger.info("=== QUESTIONS INDEX CHECK ===")
            ...
            if not knowledge_base_id:
                logger.info(
                    "No knowledge base ID provided, cannot search questions index"
                )
                return None

            # Get the questions vector store
            logger.info(
                f"Getting vector store for questions index: {settings.PINECONE_QUESTIONS_INDEX_NAME}"
            )
            ...
            logger.info(
                f"Searching questions with: query='{query}', knowledge_base_id='{knowledge_base_id}', threshold={QUESTIONS_SIMILARITY_THRESHOLD}"
            )
            ...
            if results and len(results) > 0:
                ...
                logger.info(
                    f"Found question match with score {match_score} (threshold: {QUESTIONS_SIMILARITY_THRESHOLD})"
                )
                ...
                if match_score >= QUESTIONS_SIMILARITY_THRESHOLD:
                    ...
                    if not matched_question or not matched_answer:
                        logger.info(
                            "Question/answer not found in metadata, trying to parse from content"
                        )
                        parts = content.split("\nAnswer: ")
                        if len(parts) == 2:
                            ...
                            logger.warning(
                                f"Could not parse question/answer from content: {content}"
                            )
                            return None
                    ...
                    logger.info(
                        "Generating answer with LLM using matched question and answer"
                    )
...
"""
                    ...
                    logger.info(
                        "Returning LLM-generated answer from questions index match"
                    )
                    logger.info("=== END QUESTIONS INDEX CHECK ===")
                    return response
                else:
                    logger.info(
                        f"Match score {match_score} below threshold {QUESTIONS_SIMILARITY_THRESHOLD}, not using"
                    )
            else:
            ...
            logger.info("=== END QUESTIONS INDEX CHECK (ERROR) ===")
            return None
        ...
        try:
            ...
            if service == "tag" and confidence < TAG_CONFIDENCE_THRESHOLD:
                ...
                routing_info["reasoning"] = (
                    f"Original choice was TAG with confidence {confidence}, but this is below threshold {TAG_CONFIDENCE_THRESHOLD}. Defaulting to RAG."
                )
                routing_info["fallback"] = True
    ...
    async def get_relevant_knowledge_bases(
