...
class QuestionService:
    ...
    async def create_question(
        self, kb_id: str, payload: QuestionCreate, current_user: UserResponse
    ) -> QuestionResponse:
    ...
    async def get_question(
        self, question_id: str, current_user: UserResponse
    ) -> QuestionResponse:
        ...
        await self.kb_service.get_knowledge_base(
            question.knowledge_base_id, current_user
        )

        return question

    async def list_questions(
        self, kb_id: str, current_user: UserResponse, skip: int = 0, limit: int = 100
    ...
    async def update_question(
        ...
        if "question" in update_data or "answer" in update_data:
            self.celery_app.send_task(
                "app.worker.tasks.initiate_question_ingestion", args=[question_id]
            )
