...
class KnowledgeBaseService:
    ...
    async def create_knowledge_base(
        self, kb_data: KnowledgeBaseCreate, current_user: UserResponse
    ) -> KnowledgeBase:
        ...
        try:
            ...
            kb = await self.repository.create(knowledge_base, self.db)
            return kb
        except Exception as e:
    ...
    async def get_knowledge_base(
        self, kb_id: str, current_user: UserResponse
    ) -> KnowledgeBaseResponse:
        ...
        if str(kb.user_id) != str(current_user.id):
            ...
            if (
                ...
                is_shared = await self.repository.is_shared_with_user(
                    kb_id, current_user.id, self.db
                )
                if not is_shared:
                    raise HTTPException(
                        status_code=403,
    ...
    async def update_knowledge_base(
        self, kb_id: str, kb_data: KnowledgeBaseUpdate, current_user: UserResponse
    ) -> KnowledgeBase:
        ...
        if (
            str(kb.user_id) != str(current_user.id)
        ...
        try:
            updated_kb = await self.repository.update(
                self.db, kb_id=kb_id, name=kb_data.name, description=kb_data.description
            )
            return updated_kb
        except Exception as e:
    ...
    async def share_knowledge_base(
        self, kb_id: str, user_id: str, current_user: UserResponse
    ) -> bool:
        ...
        if (
            ...
            raise HTTPException(
                status_code=403,
                detail="You don't have permission to share this knowledge base",
            )
        ...
        is_already_shared = await self.repository.is_shared_with_user(
            kb_id, user_id, self.db
        )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to share knowledge base: {str(e)}"
            )
    ...
    async def unshare_knowledge_base(
        ...
        if (
            ...
            raise HTTPException(
                status_code=403,
                detail="You don't have permission to modify sharing for this knowledge base",
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to unshare knowledge base: {str(e)}"
            )

    async def list_shared_users(self, kb_id: str, current_user: UserResponse) -> List:
        ...
        if (
            ...
            raise HTTPException(
                status_code=403,
                detail="You don't have permission to view sharing information for this knowledge base",
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to list shared users: {str(e)}"
            )
    ...
    async def list_shared_knowledge_bases(
        ...
        try:
            shared_kbs = await self.repository.list_shared_with_user(
                current_user.id, self.db
            )
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to list shared knowledge bases: {str(e)}",
            )
