...
class RerankerFactory:
    ...
    def create(config: Dict[str, Any]) -> Reranker:
        ...
        if reranker_type == "pinecone":
            ...
            logger.warning(
                f"Unknown reranker type: {reranker_type}. Falling back to CrossEncoderReranker."
            )
            return RerankerFactory._create_cross_encoder_reranker(config)
    ...
    def _create_pinecone_reranker(config: Dict[str, Any]) -> Reranker:
        ...
        if not PINECONE_AVAILABLE:
            logger.warning(
                "Pinecone not installed. Falling back to CrossEncoderReranker."
            )
            return RerankerFactory._create_cross_encoder_reranker(config)
    ...
    def _create_cross_encoder_reranker(config: Dict[str, Any]) -> Reranker:
        ...
        # Create and store the instance
        RerankerFactory._cross_encoder_instance = CrossEncoderReranker(
...
def create_reranker(config: Dict[str, Any]) -> Reranker:
    ...
    """
    logger.info(
        "Using create_reranker (deprecated). Consider using RerankerFactory.create() instead."
    )
