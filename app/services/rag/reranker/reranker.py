...
class Reranker(ABC):
    ...
    @abstractmethod
    async def rerank(
        self, query: str, chunks: List[Dict[str, Any]], top_k: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
...
class CrossEncoderReranker(Reranker):
    ...
    def __init__(self, model_name: str = "cross-encoder/ms-marco-MiniLM-L-6-v2"):
        ...
        try:
            ...
            if torch.cuda.is_available():
                logger.info(
                    "CUDA is available but forcing CPU usage to avoid segmentation faults"
                )
            ...
            logger.error(
                f"Failed to initialize CrossEncoderReranker: {e}", exc_info=True
            )
            raise
    ...
    async def rerank(
        ...
        try:
            ...
            logger.info(
                f"Reranking complete. Top score: {result[0]['rerank_score'] if result else 0}"
            )
