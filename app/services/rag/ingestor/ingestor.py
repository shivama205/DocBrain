...
from app.services.llm.factory import CompletionOptions, LLMFactory, Message, Role
    ...
    async def ingest(self, content: bytes, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            content_stream = DocumentStream(
                name=metadata.get("title", "temp.pdf"), stream=io.BytesIO(content)
            )
            ...
            docling_metadata = (
                conv_result.document.metadata.model_dump()
                if hasattr(conv_result.document, "metadata")
                else {}
            )
            ...
            for page in conv_result.document.pages:
                page_text = (
                    page.export_to_text() if hasattr(page, "export_to_text") else ""
                )
            ...
            logger.info(
                f"Successfully ingested PDF with {len(conv_result.document.pages)} pages using docling v2"
            )
            ...
            try:
                ...
                logger.info(
                    f"Successfully ingested PDF with {len(pdf_reader.pages)} pages using PyPDF2 fallback"
                )
                ...
                logger.error(
                    f"Fallback PDF ingestion also failed: {fallback_error}",
                    exc_info=True,
                )
                raise
...
class CSVIngestor(Ingestor):
    ...
    async def ingest(self, content: bytes, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            table_name = metadata.get(
                "document_title",
                f"csv_data_{datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}",
            ).replace(".csv", "")
            create_table_query = await CSVIngestor.generate_create_table_query(
                table_name, headers, rows[1:10]
            )

            # insert into storage database as new table
            storage_session = get_storage_db().__next__()

            await StorageRepository.insert_csv(
                storage_session, table_name, create_table_query, headers, rows[1:]
            )

            logger.info(
                f"Successfully ingested CSV with {len(rows) - 1 if rows else 0} rows and {len(headers)} columns"
            )
    ...
    @staticmethod
    async def generate_create_table_query(
        table_name: str, headers: List[str], rows: List[List[str]]
    ) -> str:
        """
...
class MarkdownIngestor(Ingestor):
    ...
    async def ingest(self, content: bytes, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"text": md_text, "metadata": enhanced_metadata, "html": html}
...
class ImageIngestor(Ingestor):
    ...
    async def ingest(self, content: bytes, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            content_stream = DocumentStream(
                name=metadata.get("title", "temp.png"), stream=io.BytesIO(content)
            )

            # Set up the document converter with IMAGE format options
            doc_converter = DocumentConverter(allowed_formats=[InputFormat.IMAGE])
            ...
            logger.info(
                f"Successfully ingested image with docling v2 OCR, extracted {len(text)} characters"
            )

            return {"text": text, "metadata": enhanced_metadata}
            ...
            try:
                ...
                logger.info(
                    f"Successfully ingested image with pytesseract fallback, extracted {len(text)} characters"
                )
                ...
                logger.error(
                    f"Fallback image OCR also failed: {fallback_error}", exc_info=True
                )
                raise
...
class TextIngestor(Ingestor):
    ...
    async def ingest(self, content: bytes, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Successfully ingested text with {enhanced_metadata['line_count']} lines and {enhanced_metadata['word_count']} words"
            )
...
class DocxIngestor(Ingestor):
    ...
    async def ingest(self, content: bytes, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("Successfully ingested DOCX document using docling v2")
...
class PptxIngestor(Ingestor):
    ...
    async def ingest(self, content: bytes, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            doc_converter = DocumentConverter(allowed_formats=[InputFormat.PPTX])
            ...
            logger.info(
                f"Successfully ingested PPTX document with {slide_count} slides using docling v2"
            )
...
class HTMLIngestor(Ingestor):
    ...
    async def ingest(self, content: bytes, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("Successfully ingested HTML document using docling v2")
            ...
            try:
                ...
                # Enhance metadata
                enhanced_metadata = {**metadata, "document_type": "html"}

                logger.info("Successfully ingested HTML with fallback method")
                ...
                logger.error(
                    f"Fallback HTML ingestion also failed: {fallback_error}",
                    exc_info=True,
                )
                raise
...
def html_to_markdown(html_text):
    ...
    async def ingest(self, content: bytes, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            page_texts = []
            if input_format == InputFormat.PDF and hasattr(
            ...
            logger.info(
                f"Successfully ingested {file_extension.upper()} document using docling v2"
            )
            ...
            logger.error(
                f"Failed to ingest document with docling v2: {e}", exc_info=True
            )
            ...
            try:
                ...
                logger.error(
                    f"Fallback ingestion also failed: {fallback_error}", exc_info=True
                )
                ...
                try:
                    ...
                    raise Exception(
                        "Failed to ingest document with any available method"
                    )
