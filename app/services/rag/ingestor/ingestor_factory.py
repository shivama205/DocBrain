...
from app.services.rag.ingestor.ingestor import (
    CSVIngestor,
    ImageIngestor,
    Ingestor,
    MarkdownIngestor,
    ...
    def create_ingestor(content_type: DocumentType) -> Ingestor:
        ...
        try:
            ...
            if content_type == DocumentType.PDF:
                ...
                if IngestorFactory._text_ingestor is None:
                    logger.warning(
                        f"No specific ingestor for content type {content_type}, creating new TextIngestor"
                    )
                    IngestorFactory._text_ingestor = TextIngestor()
                else:
                    logger.warning(
                        f"No specific ingestor for content type {content_type}, returning existing TextIngestor"
                    )
                return IngestorFactory._text_ingestor
